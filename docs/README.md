# Bitnami Kubernetes Production Runtime Documentation

## Installation

* [Installation of `kubeprod` binary](install.md)
* [AKS Quickstart](quickstart-aks.md)
* [GKE Quickstart](quickstart-gke.md)

## Cluster Admin Workflow and Advanced Guides

* [BKPR cluster admin workflow](workflow.md)
* [Tweaking the default configuration](overrides.md)
* [BKPR components details](components.md)
* [BKPR versioning](versioning.md)
* [Deploy Kubeapps on top of BKPR](kubeapps-on-bkpr.md)

## Migration Guides

* [BKPR 1.1 -> BKPR 1.2](migration-guides/bkpr-1.2-migration.guide.md)
* [Migration of Prometheus timeseries database](migration-guides/prometheus-migration.md)
  
## BKPR Project Internals

* [Roadmap](roadmap.md)
* [Release process](release-process.md)
* [Contributing](../CONTRIBUTING.md)
* [Code of Conduct](../CODE_OF_CONDUCT.md)

## Application Developers Guides

* [Application Developers Reference Guide](application-developers-reference-guide.md)

## Troubleshooting and FAQ

* [Trobleshooting](troubleshooting.md)
* [Frequently Asked Questions](FAQ.md)
